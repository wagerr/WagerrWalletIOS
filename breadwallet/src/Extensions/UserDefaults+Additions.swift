...
private let hasPromptedSyncWarningKey = "hasPromptedSyncWarningKey"
...
private let defaultBetAmountKey = "defaultBetAmountKey"
...
extension UserDefaults {
    ...
    static var hasPromptedSyncWarning: Bool {
        get { return defaults.bool(forKey: hasPromptedSyncWarningKey) }
        set { defaults.set(newValue, forKey: hasPromptedSyncWarningKey) }
    }
}
...
extension UserDefaults {
    ...
    static var defaultBetAmount: Int {
        get {
            var ret = defaults.integer(forKey: defaultBetAmountKey)
            if ret == 0 {
                ret = Int(W.BetAmount.min)
            }
            return ret
        }
        set { defaults.setValue(newValue, forKey: defaultBetAmountKey) }
    }
}
